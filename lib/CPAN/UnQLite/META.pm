...
sub new {
  ...
  my $cpan_sqlite = CPAN::UnQLite->new();
...
package CPAN::UnQLite::META::Author;
use parent 'CPAN::UnQLite::META';
use CPAN::UnQLite::Util qw(has_hash_data);
...
package CPAN::UnQLite::META::Distribution;
use parent 'CPAN::UnQLite::META';
use CPAN::UnQLite::Util qw(has_hash_data download);
...
package CPAN::UnQLite::META::Module;
use parent 'CPAN::UnQLite::META';
use CPAN::UnQLite::Util qw(has_hash_data);
...
package CPAN::UnQLite::META::Bundle;
use parent 'CPAN::UnQLite::META';
use CPAN::UnQLite::Util qw(has_hash_data);
...
sub reload {
  ...
  my $force = $args{force};
  my $db_name = $CPAN::UnQLite::db_name;
  ...
  my @args = ($^X, '-MCPAN::UnQLite::META=setup,update,check', '-e');
  if (-e $db && -s _) {
  ...
  if ($CPAN::UnQLite::DBI::dbh) {
    $CPAN::UnQLite::DBI::dbh->disconnect();
    $CPAN::UnQLite::DBI::dbh = undef;
  }
...
sub setup {
  my $obj = CPAN::UnQLite->new(setup => 1);
  $obj->index() or die qq{CPAN::UnQLite setup failed};
  return;
}

sub update {
  my $obj = CPAN::UnQLite->new();
  $obj->index() or die qq{CPAN::UnQLite update failed};
  return;
}
...
sub check {
  ...
  my $dbh = DBI->connect("DBI:UnQLite:$db", '', '', {'RaiseError' => 0, 'PrintError' => 0, 'AutoCommit' => 1});
...
as a helper module for use of CPAN::UnQLite within the CPAN.pm
module. A new object is created as

  my $obj = CPAN::UnQLite::META->new($CPAN::META);
...
identify the class. The method searches the C<CPAN::UnQLite>
...
where C<$results> are the results returned from C<CPAN::UnQLite>.
