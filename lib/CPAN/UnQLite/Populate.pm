...
use CPAN::UnQLite::Util qw($table_id has_hash_data print_debug);
use CPAN::UnQLite::DBI::Index;
...
sub new {
  ...
  foreach my $table (@tables) {
    my $obj = $index->{$table};
    die "Please supply a CPAN::UnQLite::Index::$table object"
      unless ($obj and ref($obj) eq "CPAN::UnQLite::Index::$table");
  }
  my $state = $args{state};
  unless ($setup) {
    die "Please supply a CPAN::UnQLite::State object"
      unless ($state and ref($state) eq 'CPAN::UnQLite::State');
  }
  my $cdbi = CPAN::UnQLite::DBI::Index->new(%args);
...
sub create_objs {
  ...
  foreach my $table (@tables) {
    ...
    if ($index and ref($index) eq "CPAN::UnQLite::Index::$table") {
      my $info = $index->{info};
      if ($table ne 'info') {
...
package CPAN::UnQLite::Populate::auths;
use parent 'CPAN::UnQLite::Populate';
use CPAN::UnQLite::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::UnQLite::DBI::Index::auths');
  my $self = {
              info => $info,
              insert => {},
              update => {},
...
package CPAN::UnQLite::Populate::dists;
use parent 'CPAN::UnQLite::Populate';
use CPAN::UnQLite::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  my $cdbi = $args{cdbi};
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::UnQLite::DBI::Index::dists');
  my $self = {
              info => $info,
              insert => {},
              update => {},
...
package CPAN::UnQLite::Populate::mods;
use parent 'CPAN::UnQLite::Populate';
use CPAN::UnQLite::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  my $cdbi = $args{cdbi};
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::UnQLite::DBI::Index::mods');
  my $self = {
              info => $info,
              insert => {},
              update => {},
...
package CPAN::UnQLite::Populate::chaps;
use parent 'CPAN::UnQLite::Populate';
use CPAN::UnQLite::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  my $cdbi = $args{cdbi};
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::UnQLite::DBI::Index::chaps');
  my $self = {
              obj => {},
              cdbi => $cdbi,
              error_msg => '',
...
package CPAN::UnQLite::Populate::info;
use parent 'CPAN::UnQLite::Populate';
use CPAN::UnQLite::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  my $cdbi = $args{cdbi};
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::UnQLite::DBI::Index::info');
  my $self = {
              obj => {},
              cdbi => $cdbi,
              error_msg => '',
...
CPAN::UnQLite::Populate - create and populate database tables
...
I<CPAN::UnQLite::Info> and the
state information from I<CPAN::UnQLite::State>. It does
...
C<CPAN::UnQLite::Index> object; existing tables will be
dropped.
