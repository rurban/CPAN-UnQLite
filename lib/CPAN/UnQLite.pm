...
sub index {
  ...
  my $index = CPAN::UnQLite::Index->new(%wanted, %args, log_dir => $log_dir);
  $index->index() or do {
...
sub query {
  my ($self, %args) = @_;
  require CPAN::UnQLite::Search;
  my %wanted = map {$_ => $self->{$_}} qw(max_results CPAN db_name db_dir meta_obj);
  my $query = CPAN::UnQLite::Search->new(%wanted, %args);
...
=head1 SYNOPSIS
  ...
  my $obj = CPAN::UnQLite->new(CPAN => '/path/to/CPAN');
  $obj->index(setup => 1);
...
Using CPAN::UnQLite, what happens is that a request for information
through CPAN.pm, such as

  cpan> a ANDK

will cause a query to the UnQLite database to be made.
...
with and without CPAN::UnQLite will be essentially the same.
...
L<CPAN::UnQLite::META>. See also the L<cpandb> script for a
command-line interface to the
indexing and querying of the database.

See L<https://github.com/rurban/CPAN-UnQLite/>.

=head1 SUPPORT

You can find documentation for this module with the perldoc command.

    perldoc CPAN::UnQLite
...
L<http://annocpan.org/dist/CPAN-UnQLite>

=item * CPAN::Forum: Discussion forum

L<http:///www.cpanforum.com/dist/CPAN-UnQLite>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/CPAN-UnQLite>

=item * RT: CPAN's request tracker

L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=CPAN-UnQLite>

=item * Search CPAN

L<http://search.cpan.org/dist/CPAN-UnQLite>

=back

=head1 BUGS

At this time, CPAN::UnQLite keeps information contained only
...
This behaviour differs from that of L<CPAN> without CPAN::UnQLite.
...
is set. This is automatically set within L<CPAN::UnQLite::Index>.
...
Copyright 2013 by Reini Urban E<lt>rurban@cpan.orgE<gt>.
